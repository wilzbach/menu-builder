...
module.exports = MenuBuilder = view.extend({
  ...
  modifyNode: function(label,callback,opts){
   ...
   node.opts = opts || node.opts;
   console.log(node.opts.style);
   console.log(this._nodes[1].opts.style);
  },
  ...
  _buildM: function(data) {
    ...
    for (_i = 0, _len = nodes.length; _i < _len; _i++) {
      ...
      if (this.config != null) {
        li.style.lineHeight = this.config.menuItemLineHeight;
      }
    ...
    displayedButton.className = "s-menubar_alink";
    if (this.config != null) {
      menuUl.style.fontSize = this.config.menuItemFontsize;
      displayedButton.style.fontSize = this.config.menuFontsize;
      displayedButton.style.marginLeft = this.config.menuMarginLeft;
      displayedButton.style.padding = this.config.menuPadding;
    }
